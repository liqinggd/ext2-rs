...
use crate::{
    inode::RawInode,
    prelude::*,
    utils::{FixedStr, IsPowerOf, UnixTime},
};
...
impl TryFrom<RawSuperBlock> for SuperBlock {
    ...
    fn try_from(sb: RawSuperBlock) -> Result<Self> {
        Ok(Self {
            ...
            state: {
                let state = FsState::try_from(sb.state).map_err(|_| FsError::WrongFs)?;
                if state == FsState::Corrupted {
                    return Err(FsError::WrongFs);
                }
                state
            },
            errors_behaviour: ErrorsBehaviour::try_from(sb.errors).map_err(|_| FsError::WrongFs)?,
            last_check_time: sb.last_check_time,
            check_interval: Duration::from_secs(sb.check_interval as _),
            creator_os: {
                let os_id = OsId::try_from(sb.creator_os).map_err(|_| FsError::WrongFs)?;
                if os_id != OsId::Linux {
                    return Err(FsError::WrongFs);
                }
                OsId::Linux
            ...
            rev_level: {
                let rev_level = RevLevel::try_from(sb.rev_level).map_err(|_| FsError::WrongFs)?;
            ...
            feature_compat: FeatureCompatSet::from_bits(sb.feature_compat)
                .ok_or(FsError::WrongFs)?,
            feature_incompat: FeatureInCompatSet::from_bits(sb.feature_incompat)
                .ok_or(FsError::WrongFs)?,
            feature_ro_compat: FeatureRoCompatSet::from_bits(sb.feature_ro_compat)
                .ok_or(FsError::WrongFs)?,
            uuid: sb.uuid,
...
impl SuperBlock {
    ...
    pub(super) fn bid(&self, block_group_idx: usize) -> u32 {
        if block_group_idx == 0 {
    ...
    /// If `block_group_idx` is neither 0 nor a backup block group index,
    /// then the method panics.
    pub(super) fn group_descriptors_bid(&self, block_group_idx: usize) -> u32 {
...
impl TryFrom<u16> for FsState {
    ...
    fn try_from(val: u16) -> Result<Self> {
        match val {
            val if val == Self::Valid as u16 => Ok(Self::Valid),
            val if val == Self::Err as u16 => Ok(Self::Err),
            val if val == Self::Corrupted as u16 => Ok(Self::Corrupted),
...
impl TryFrom<u16> for ErrorsBehaviour {
    type Error = FsError;

    fn try_from(val: u16) -> Result<Self> {
        match val {
            val if val == Self::Continue as u16 => Ok(Self::Continue),
            val if val == Self::RemountReadonly as u16 => Ok(Self::RemountReadonly),
            val if val == Self::Panic as u16 => Ok(Self::Panic),
            _ => Err(FsError::InvalidParam),
        }
    }
}
...
impl TryFrom<u32> for OsId {
    type Error = FsError;

    fn try_from(val: u32) -> Result<Self> {
        match val {
            val if val == Self::Linux as u32 => Ok(Self::Linux),
            val if val == Self::Hurd as u32 => Ok(Self::Hurd),
            val if val == Self::Masix as u32 => Ok(Self::Masix),
            val if val == Self::FreeBSD as u32 => Ok(Self::FreeBSD),
            val if val == Self::Lites as u32 => Ok(Self::Lites),
            _ => Err(FsError::InvalidParam),
        }
    }
}
...
impl TryFrom<u32> for RevLevel {
    type Error = FsError;

    fn try_from(val: u32) -> Result<Self> {
        match val {
            val if val == Self::GoodOld as u32 => Ok(Self::GoodOld),
            val if val == Self::Dynamic as u32 => Ok(Self::Dynamic),
            _ => Err(FsError::InvalidParam),
        }
    }
}
