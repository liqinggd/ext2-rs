...
impl BlockGroup {
        ...
        let metadata = {
            let descriptor = {
                ...
                let (blk_idx, blk_offset) = {
                    let offset = idx * size_of::<GroupDescriptor>();
                    (offset / BLOCK_SIZE, offset % BLOCK_SIZE)
                };
                group_descriptors[blk_idx].read_val::<GroupDescriptor>(blk_offset)?
            };

            let get_bitmap = |bid: u32, capacity: usize| -> Result<IdAlloc> {
                if capacity > BLOCK_SIZE * 8 {
                    return Err(FsError::WrongFs);
                }
                let mut buf = BlockBuf::new_uninit();
                block_device.read_block(bid as Bid, buf.as_mut_slice())?;
                Ok(IdAlloc::from_bytes_with_capacity(buf.as_slice(), capacity))
            };
    ...
    /// Loads an existing inode from block device.
    fn load_inode(&self, inode_idx: u32) -> Result<Arc<Inode>> {
        let fs = self.fs();
        let raw_inode = {
            let offset = (inode_idx as usize) * fs.inode_size()
                + (self.inode_table_bid as usize) * BLOCK_SIZE;
            fs.block_device().read_val::<RawInode>(offset)?
        };
    ...
    pub fn alloc_inode(&self, is_dir: bool) -> Option<u32> {
        // The fast path
        if self.inner.read().metadata.free_inodes_count() == 0 {
            return None;
        }
    ...
    pub fn alloc_blocks(&self, count: u32) -> Option<Range<u32>> {
        // The fast path
        if self.inner.read().metadata.free_blocks_count() == 0 {
            return None;
        }
    ...
    pub fn sync_raw_inode(&self, inode_idx: u32, raw_inode: &RawInode) -> Result<()> {
        let fs = self.fs();
        let offset =
            (inode_idx as usize) * fs.inode_size() + (self.inode_table_bid as usize) * BLOCK_SIZE;
        fs.block_device().write_val(offset, raw_inode)?;
        Ok(())
    }

    /// Writes back the metadata of this group.
    pub fn sync_metadata(&self) -> Result<()> {
        if !self.inner.read().metadata.is_dirty() {
            return Ok(());
        }
    ...
    pub fn sync_all_inodes(&self) -> Result<()> {
        ...
        let remaining_inodes: Vec<Arc<Inode>> =
            self.inner.read().inode_cache.values().cloned().collect();
...
impl Debug for BlockGroup {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        f.debug_struct("BlockGroup")
            .field("idx", &self.idx)
            .field("descriptor", &self.inner.read().metadata.descriptor)
            .field("block_bitmap", &self.inner.read().metadata.block_bitmap)
            .field("inode_bitmap", &self.inner.read().metadata.inode_bitmap)
            .finish()
    }
}
...
///
/// The descriptor table starts on the first block following the superblock.
#[repr(C)]
#[derive(Clone, Copy, Debug, Pod)]
pub(super) struct GroupDescriptor {
    /// Blocks usage bitmap block
    block_bitmap_bid: u32,
    /// Inodes usage bitmap block
    inode_bitmap_bid: u32,
...
const_assert!(size_of::<GroupDescriptor>() == 32);
